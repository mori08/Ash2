...
- 画面座標は `toScreen()` で `Vec2{ w, -(d * kDepthScale + h) }`。`kDepthScale` は奥行きの
  画面投影の圧縮率（0.5）で、奥行き方向の移動量は画面上で高さ方向の半分になる。`HitSystem` の
  当たり判定など、ワールド空間で行う判定は圧縮せず w-d 平面で等方のまま扱う。一方
  `LockOnSystem::Contains` は `WorldToScreen` で投影した後の画面空間で判定しており、圧縮の
  影響を受ける。描画順は `d` の降順（奥から手前）
- 高さと奥行きは圧縮された上で同じ画面軸に潰れるが、当たり判定は3軸を保ったまま行う。描画で
  潰れる `h` と `d` を、判定では区別する
- ワールド原点の画面位置（床原点）は `WorldOrigin()`（`Screen.hpp`）が返す。`Scene::Center()`
  から `kFloorOriginOffsetY` だけ下にずらし、床を画面下半分に収める
