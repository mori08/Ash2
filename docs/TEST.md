...
1つのテスト対象につき1ファイルとし、`Test<対象名>.cpp` の名前をつける。
別の対象のテストを同じファイルに混ぜない。
...
テストは `RegisterAssets()` とゲームループの手前で走り切る（`Ash2/src/Main.cpp`）。
テスト実行中はアセットが登録されておらず、フレームも回らず、入力も取得されていない。
...
準備は各テストファイル内の匿名 namespace にヘルパーとして置く。
2つ以上のファイルで同じヘルパーが必要になった時点で `tests/Support/` へ切り出す。
先回りして共通化しない。

Config のように全フィールドの初期化が要るものは、既定値を返すヘルパーを1つ用意し、
各テストは差分だけを上書きする。
